...
def draw_text(text, surface, size=15, x=0, y=0):  # display score
    score_text = pygame.font.SysFont('arial', size)
    text_surf, text_rect = text_objects(text, score_text)
    text_rect.left = x
    text_rect.top = y
...
def draw_body(snake, surface):
    ...
    for block in sorted(snake.body_dict.keys(), reverse=True): # reverse order to see where head crashes
...
def draw_food(food_block, surface, color=colors['white']):
...
def draw_window(surface, snake, food_block, score, show_grid):
    ...
    draw_text(f'Score: {score}', surface, y=30) # display the score
...
class FoodBlock():
    ...
    def __init__(self, snake, x_max, y_max):
        ...
        self.pixels_to_avoid = list(snake.body_dict.values())

        self.reset_block(snake)

    def reset_block(self, snake):  # resets food block to a new location
        ...
        self.pixels_to_avoid = list(snake.body_dict.values())
        avail_positions = []
        for x in range(0, self.x_max, self.block_size):
            for y in range(0, self.y_max, self.block_size):
                if [x,y] not in self.pixels_to_avoid:
                    avail_positions.append([x, y])


        self.position = random.choice(avail_positions)
...
class Snake:
    ...
    def __init__(self, x, y, max_x, max_y, velocity=20, block_size=20, direction='up', store_past_n_moves=3):
        ...
        self.last_n_actions = deque([0 for i in range(store_past_n_moves)]) # [most recent, ..., oldest]

    def add_body_block(self):
        # add another block temporarily offscreen for current frame
        self.body_dict[self.number_blocks] = [-60, -60] # instantiate offscren
        self.number_blocks += 1
    ...
    def action(self, action_num):
        ...
        # update last n actions
        self.last_n_actions[-1] = action_num
        self.last_n_actions.rotate(1)
